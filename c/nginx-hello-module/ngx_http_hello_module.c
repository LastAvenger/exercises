 ...
 * Nginx Hello Module
 *
 * Ref: http://tengine.taobao.org/book/chapter_03.html#hello-handler
 * Usage:
 *
 *      location /test {
 *          hello_string test;
...
static ngx_int_t ngx_helll_visited_times = 0;
...
ngx_int_t ngx_http_hello_handler(ngx_http_request_t *r) {
    ...
    if (my_conf->hello_counter == NGX_CONF_UNSET
        ...
        ngx_sprintf(ngx_hello_string, "%s, Visited Times: %d",
                my_conf->hello_string.data, ++ngx_helll_visited_times);
    }

    ngx_log_error(NGX_LOG_EMERG, r->connection->log, 0,
            "hello_string: %s", ngx_hello_string);
